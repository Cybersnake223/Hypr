...
PanelWindow {
    ...
    property string configHome: Quickshell.env("HOME") + "/.config"
    ...
    Component.onCompleted: {
        m.append({ label: "aliases", path: root.configHome + "/zsh/.zsh_aliases" })
        m.append({ label: "anyrun", path: root.configHome + "/anyrun/config.ron" })
        m.append({ label: "cava", path: root.configHome + "/cava/config" })
        m.append({ label: "foot", path: root.configHome + "/foot/foot.ini" })
        m.append({ label: "hyprland", path: root.configHome + "/hypr/hyprland.lua" })
        m.append({ label: "hyprlock", path: root.configHome + "/hypr/hyprlock.conf" })
        m.append({ label: "hyprpaper", path: root.configHome + "/hypr/hyprpaper.conf" })
        m.append({ label: "kitty", path: root.configHome + "/kitty/kitty.conf" })
        m.append({ label: "mako", path: root.configHome + "/mako/config" })
        m.append({ label: "matugen", path: root.configHome + "/matugen/config.toml" })
        m.append({ label: "mpv", path: root.configHome + "/mpv/mpv.conf" })
        m.append({ label: "mpv-input", path: root.configHome + "/mpv/input.conf" })
        m.append({ label: "nvim", path: root.configHome + "/nvim/init.lua" })
        m.append({ label: "rofi", path: root.configHome + "/rofi/config.rasi" })
        m.append({ label: "awww", path: root.configHome + "/awww/config" })
        m.append({ label: "waybar", path: root.configHome + "/waybar/config.jsonc" })
        m.append({ label: "waybar-style", path: root.configHome + "/waybar/style.css" })
        m.append({ label: "yay", path: root.configHome + "/yay/config.json" })
        m.append({ label: "yazi", path: root.configHome + "/yazi/yazi.toml" })
        m.append({ label: "yt-dlp", path: root.configHome + "/yt-dlp/config" })
        m.append({ label: "zsh", path: root.configHome + "/zsh/.zshrc" })
    ...
    Process {
        ...
        command: ["bash", "-c",
            "path=" + JSON.stringify(editProc.path) + "; " +
            "if [ ! -f \"$path\" ]; then " +
            "confirm=$(printf 'Create\\nCancel' | rofi -dmenu -p 'File not found: '); " +
            ...
            RowLayout {
                ...
                Text {
                    ...
                    color: searchInput.activeFocus
                           ? Qt.rgba(theme.mauve.r, theme.mauve.g, theme.mauve.b, 0.7)
                           : theme.subtext0
                    font.pixelSize: 14
                    opacity: 0.5
                }
