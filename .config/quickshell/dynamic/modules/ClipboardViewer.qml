...
PanelWindow {
    ...
    function copyItem(idx) {
        if (idx < 0 || idx >= filteredItems.count)
            return;
        ...
        stdout: StdioCollector {
            onStreamFinished: {
                ...
                for (let i = 0; i < lines.length; i++) {
                    ...
                    let label = isBinary ? (preview.match(/\[\[.*?(\d+x\d+)/) ? "[ image " + preview.match(/(\d+x\d+)/)[1] + " ]" : "[ image ]") : preview;
                    allItems.append({
                        id,
                        preview: label,
                        isBinary
    ...
    Process {
        ...
        command: ["bash", "-c", "cliphist list | awk -v id=" + JSON.stringify(copyProc.itemId) + " -F'\\t' '$1==id' | cliphist decode | wl-copy"]
    }
            ...
            Column {
                    ...
                    Item {
                        ...
                        TextInput {
                            ...
                            Keys.onReturnPressed: function (e) {
                                root.copyItem(list.currentIndex);
                                e.accepted = true;
                            }
                ...
                ListView {
                    ...
                    delegate: Rectangle {
                        ...
                        height: s(54)
                        radius: s(12)
                        color: list.currentIndex === index ? Qt.rgba(theme.surface0.r, theme.surface0.g, theme.surface0.b, 0.85) : (rowHover.containsMouse ? Qt.rgba(theme.surface0.r, theme.surface0.g, theme.surface0.b, 0.4) : "transparent")
                        Behavior on color {
                            ColorAnimation {
                                duration: 100
                            }
                        ...
                        Row {
                            ...
                            Rectangle {
                                ...
                                radius: s(8)
                                anchors.verticalCenter: parent.verticalCenter
                                color: model.isBinary ? Qt.rgba(theme.mauve.r, theme.mauve.g, theme.mauve.b, 0.15) : Qt.rgba(theme.teal.r, theme.teal.g, theme.teal.b, 0.12)
                            ...
                            Text {
                                ...
                                font.family: SharedConfig.monoFont
                                font.pixelSize: s(13)
                                font.weight: model.isBinary ? Font.Normal : Font.Medium
                                color: list.currentIndex === index ? theme.text : (model.isBinary ? theme.subtext0 : Qt.rgba(theme.text.r, theme.text.g, theme.text.b, 0.8))
